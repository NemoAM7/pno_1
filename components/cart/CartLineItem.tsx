  ...
  return (
    <article className="flex gap-4 border-b border-line py-5">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{line.name}</h3><p className="mt-1 text-sm text-ink/70">{[line.color, line.size].filter(Boolean).join(' / ')}</p></div><p className="font-bold">{formatPrice(line.unitCents * line.quantity)}</p></div>
