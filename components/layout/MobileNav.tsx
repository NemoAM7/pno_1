...
export function MobileNav() {
  ...
  return (
    <div className="md:hidden">
      <button type="button" onClick={() => setOpen((current) => !current)} className="flex h-10 w-10 flex-col items-center justify-center gap-1 rounded-full border border-ink" aria-label={open ? 'Close navigation menu' : 'Open navigation menu'} aria-expanded={open} aria-controls="mobile-navigation"><span className={`block h-0.5 w-4 bg-ink transition-transform ${open ? 'translate-y-1.5 rotate-45' : ''}`} /><span className={`block h-0.5 w-4 bg-ink transition-opacity ${open ? 'opacity-0' : ''}`} /><span className={`block h-0.5 w-4 bg-ink transition-transform ${open ? '-translate-y-1.5 -rotate-45' : ''}`} /></button>
